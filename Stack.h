...
private:
	struct Node //Узел, в котором храниться элемент стекаа и указатель на следующий
	{
		AnyT Data;
		Node* pNext;
	};
...
template<class AnyT>
Stack<AnyT>::~Stack()//Деструктор, удоляет от головы стека до конца, обнуление длинны
{
	while (pHead) {
		Node* pDelete = pHead;
		pHead = pHead->pNext;
